...
<html>
	...
	<body>
		...
		$permalink = get_permalink($post->post_parent); if(strpos($permalink, 'muuseum')) { $color = 'muuseum'; } elseif(strpos($permalink, 'selts')) { $color = 'selts'; } else { $color = 'kyla'; } ?>
...
*/
			...
			$output = '';
			if (is_page( )) {
				$page = $post->ID; if ($post->post_parent) { $page = $post->post_parent; } $children=wp_list_pages( 'echo=0&depth=1&child_of=' . $page . '&title_li=' ); if ($children) { $output = wp_list_pages ('echo=0&depth=1&child_of=' . $page . '&title_li='); } } //echo $output; ?>
			...
			<div class="prefix_6 grid_4 suffix_6 center">
				<?
        	// logo vahetaja
        	$permalink = get_permalink($post->post_parent); if(strpos($permalink, 'muuseum')) { $logo = 'rootsi_kallavere_kula_muuseum.png'; } elseif(strpos($permalink, 'selts')) { $logo = 'rootsi_kallavere_kula_selts.png'; } else { $logo = 'kallavere_kula.png'; } ?>
